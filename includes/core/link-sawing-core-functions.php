 ...
 * Core functions
 */
class link_sawing_Core_Functions
{
	...
	function init_hooks()
	{
		...
		// Trailing slashes
		add_filter('link_sawing_filter_final_term_permalink', array($this, 'control_trailing_slashes'), 9);
		add_filter('link_sawing_filter_final_post_permalink', array($this, 'control_trailing_slashes'), 9);
		add_filter('link_sawing_filter_post_sample_uri', array($this, 'control_trailing_slashes'), 9);
		...
		if (!empty($link_sawing_options['general']['case_insensitive_permalinks'])) {
			add_action('parse_request', array($this, 'case_insensitive_permalinks'), 0);
		}
		// Force 404 on non-existing pagination pages
		if (!empty($link_sawing_options['general']['pagination_redirect'])) {
	...
	{
		global $wp, $wp_rewrite, $link_sawing_uris, $link_sawing_options, $pm_query;
		...
		if (filter_var($request_url, FILTER_VALIDATE_URL)) {
			...
			$deep_detect_enabled = apply_filters('link_sawing_deep_uri_detect', true);
			...
			$endpoints = link_sawing_Helper_Functions::get_endpoints();
			...
			$uri_parts = apply_filters('link_sawing_detect_uri', $uri_parts, $request_url, $endpoints);
			...
			$exclude_drafts = apply_filters('link_sawing_exclude_drafts', $exclude_drafts);

			// Check what content type should be loaded in case of duplicate ("posts" or "terms")
			$duplicates_priority = apply_filters('link_sawing_duplicates_priority', false);
			...
			do {
				...
				$all_uris = $link_sawing_uris;

				// In case of multiple elements using the same URI, the function will follow the "link_sawing_duplicates_priority" filter value to determine whether terms or posts should be ignored
				...
				$element_id = apply_filters('link_sawing_detected_element_id', $element_id, $uri_parts, $request_url);
				...
				if (strpos($element_id, 'tax-') !== false) {
					...
					$term_element_id = apply_filters('link_sawing_detected_term_id', $term_element_id, $uri_parts, true);
					...
					$disabled = ($term_taxonomy && link_sawing_Helper_Functions::is_term_excluded($term)) ? true : false;
				 ...
				 */
					...
					// Filter detected post ID
					$post_element_id = apply_filters('link_sawing_detected_post_id', $element_id, $uri_parts);
					...
					$disabled = ($post_type && link_sawing_Helper_Functions::is_post_excluded($post_to_load)) ? true : false;
				...
				if (!empty($broken_uri) && (!empty($link_sawing_options['general']['auto_fix_duplicates'])) && $link_sawing_options['general']['auto_fix_duplicates'] == 1) {
					// Do not trigger if WP Rocket cache plugin is turned on
					if (!defined('WP_ROCKET_VERSION') && is_array($link_sawing_uris)) {
						$broken_element_id = (!empty($revision_id)) ? $revision_id : $element_id;
						$remove_broken_uri = (!empty($broken_element_id)) ? link_sawing_Actions::force_clear_single_element_uris_and_redirects($broken_element_id) : '';
				...
				$excluded = apply_filters('link_sawing_excluded_element_id', $excluded, $element_object, $old_query, $pm_query);
		...
		$query     = apply_filters('link_sawing_filter_query', $query, $old_query, $uri_parts, $pm_query, $content_type, $element_object);
	...
	static function control_trailing_slashes($permalink)
	{
		...
		$trailing_slash_setting = (!empty($link_sawing_options['general']['trailing_slashes'])) ? $link_sawing_options['general']['trailing_slashes'] : "";
		...
		return apply_filters('link_sawing_control_trailing_slashes', $permalink, $original_permalink);
	}
	...
	{
		global $wp_query, $wp, $wp_rewrite, $wpdb, $link_sawing_uris, $link_sawing_redirects, $link_sawing_external_redirects, $link_sawing_options, $pm_query;
		...
		$trailing_slashes_mode     = (!empty($link_sawing_options['general']['trailing_slashes'])) ? $link_sawing_options['general']['trailing_slashes'] : false;
		$trailing_slashes_redirect = (!empty($link_sawing_options['general']['trailing_slashes_redirect'])) ? $link_sawing_options['general']['trailing_slashes_redirect'] : false;
		$extra_redirects           = (!empty($link_sawing_options['general']['extra_redirects'])) ? $link_sawing_options['general']['extra_redirects'] : false;
		$canonical_redirect        = (!empty($link_sawing_options['general']['canonical_redirect'])) ? $link_sawing_options['general']['canonical_redirect'] : false;
		$old_slug_redirect         = (!empty($link_sawing_options['general']['old_slug_redirect'])) ? $link_sawing_options['general']['old_slug_redirect'] : false;
		$endpoint_redirect         = (!empty($link_sawing_options['general']['endpoint_redirect'])) ? $link_sawing_options['general']['endpoint_redirect'] : false;
		$pagination_redirect       = (!empty($link_sawing_options['general']['pagination_redirect'])) ? $link_sawing_options['general']['pagination_redirect'] : false;
		$copy_query_redirect       = (!empty($link_sawing_options['general']['copy_query_redirect'])) ? $link_sawing_options['general']['copy_query_redirect'] : false;
		$redirect_type             = '-';
		...
		if (!is_author() && !is_front_page() && !is_home() && !is_feed() && !is_search() && empty($_GET['s'])) {
			 ...
			 */
			if (!empty($pm_query['id']) && !empty($link_sawing_external_redirects[$pm_query['id']])) {
				$external_url = $link_sawing_external_redirects[$pm_query['id']];

				if (filter_var($external_url, FILTER_VALIDATE_URL)) {
					// Allow redirect
					$wp_query->query_vars['do_not_redirect'] = 0;

					wp_redirect($external_url, 301, link_sawing_PLUGIN_NAME);
					exit();
				}
			}
			 ...
			 * 1B. Custom redirects
			 */
			if (empty($wp_query->query_vars['do_not_redirect']) && $extra_redirects && !empty($link_sawing_redirects) && is_array($link_sawing_redirects) && !empty($wp->request) && !empty($pm_query['uri'])) {
				$uri            = $pm_query['uri'];
				...
				$force_lowercase = apply_filters('link_sawing_force_lowercase_uris', true);
				...
				foreach ($link_sawing_redirects as $element => $redirects) {
			...
			if ($canonical_redirect && empty($wp_query->query_vars['do_not_redirect']) && !empty($queried_object) && empty($correct_permalink)) {
				...
				if (!empty($queried_object->ID) && isset($link_sawing_uris[$queried_object->ID]) && empty($wp_query->query['preview'])) {
					...
					if (link_sawing_Helper_Functions::is_post_excluded($queried_object)) {
						return;
					}
				...
				else if (!empty($queried_object->term_id) && isset($link_sawing_uris["tax-{$queried_object->term_id}"]) && defined('link_sawing_PRO')) {
					// Check if the term is excluded
					if (link_sawing_Helper_Functions::is_term_excluded($queried_object)) {
						return;
					}
		 ...
		 */
		if (!empty($link_sawing_options['general']['sslwww_redirect'])) {
		 ...
		 * 7. Debug redirect
		 */
		$correct_permalink = apply_filters('link_sawing_filter_redirect', $correct_permalink, $redirect_type, $queried_object, $old_uri);
		...
		if (!empty($correct_permalink) && is_string($correct_permalink) && !empty($redirect_mode)) {
			...
			if ($redirect_type === 'www_redirect' || $rel_old_uri !== $rel_new_uri) {
				wp_safe_redirect($correct_permalink, $redirect_mode, link_sawing_PLUGIN_NAME);
				exit();
			}
		}
	}
	...
	{
		global $link_sawing_options, $wp, $wp_rewrite;
		...
		if (empty($link_sawing_options['general']['canonical_redirect'])) {
			remove_action('template_redirect', 'redirect_canonical');
		}

		if (empty($link_sawing_options['general']['old_slug_redirect'])) {
	...
	function case_insensitive_permalinks()
	{
		...
		if (!empty($_SERVER['REQUEST_URI'])) {
			...
			$link_sawing_uris = array_map('strtolower', $link_sawing_uris);
		}
	}
}
