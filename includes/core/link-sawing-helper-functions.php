 ...
 */
class link_sawing_Helper_Functions
{
	...
	public function init()
	{
		...
		add_filter('link_sawing_filter_default_post_uri', array($this, 'replace_empty_placeholder_tags'), 10, 5);
		add_filter('link_sawing_filter_default_term_uri', array($this, 'replace_empty_placeholder_tags'), 10, 5);

		// Clear the final default URIs
		add_filter('link_sawing_filter_default_term_uri', array($this, 'clear_single_uri'), 20);
		add_filter('link_sawing_filter_default_post_uri', array($this, 'clear_single_uri'), 20);
	...
	static function get_primary_term($post_id, $taxonomy, $slug_only = true)
	{
		...
		$primary_term_enabled = (isset($link_sawing_options['general']['primary_category'])) ? (bool) $link_sawing_options['general']['primary_category'] : true;
		$primary_term_enabled = apply_filters('link_sawing_primary_term', $primary_term_enabled);
	...
	static function get_term_full_slug($term, $terms, $mode = false, $native_uri = false)
	{
		...
		if ($mode == 1) {
			$term_slug = (!empty($link_sawing_uris["tax-{$term->term_id}"])) ? $link_sawing_uris["tax-{$term->term_id}"] : '';
			...
			if (!empty($term->slug)) {
				$term_slug = ($native_uri) ? $term->slug : link_sawing_Helper_Functions::force_custom_slugs($term->slug, $term);
			} else if (!empty($terms)) {
	...
	{
		global $wp_post_types, $link_sawing_options;
		...
		foreach ($wp_post_types as $post_type) {
			if (!is_post_type_viewable($post_type) || (empty($post_type->query_var) && empty($post_type->rewrite) && empty($post_type->_builtin) && !empty($link_sawing_options['general']['partial_disable_strict']))) {
		...
		if ($include_user_excluded) {
			$disabled_post_types = (!empty($link_sawing_options['general']['partial_disable']['post_types'])) ? array_merge((array) $link_sawing_options['general']['partial_disable']['post_types'], $disabled_post_types) : $disabled_post_types;
		}

		return apply_filters('link_sawing_disabled_post_types', $disabled_post_types);
	}
	...
	{
		global $wp_taxonomies, $link_sawing_options;
		...
		foreach ($wp_taxonomies as $taxonomy) {
			if (!is_taxonomy_viewable($taxonomy) || (empty($taxonomy->query_var) && empty($taxonomy->rewrite) && empty($taxonomy->_builtin) && !empty($link_sawing_options['general']['partial_disable_strict']))) {
		...
		if ($include_user_excluded) {
			$disabled_taxonomies = (!empty($link_sawing_options['general']['partial_disable']['taxonomies'])) ? array_merge((array) $link_sawing_options['general']['partial_disable']['taxonomies'], $disabled_taxonomies) : $disabled_taxonomies;
		}

		return apply_filters('link_sawing_disabled_taxonomies', $disabled_taxonomies);
	}

	/**
	 * Check if the post type should be ignored by Link Sawing
	 *
	...
	/**
	 * Check if the taxonomy should be ignored by Link Sawing
	 *
	 * @param string $taxonomy
	 * @param bool $check_if_exists
	 *
	...
	/**
	 * Check if specific post should be ignored by Link Sawing
	 *
	...
	public static function is_post_excluded($post = null, $draft_check = false)
	{
		...
		$excluded_post_ids = apply_filters('link_sawing_excluded_post_ids', array());
	...
	public static function is_draft_excluded($post = null)
	{
		...
		if (!empty($post->post_status)) {
			if (!empty($link_sawing_options["general"]["ignore_drafts"])) {
				$post_statuses = ($link_sawing_options["general"]["ignore_drafts"] == 2) ? array('draft', 'pending') : array('draft');
	...
	/**
	 * Check if specific term should be ignored by Link Sawing
	 *
	 * @param WP_Term $term
	 *
	 * @return bool
	...
	public static function is_term_excluded($term = null)
	{
		...
		$excluded_term_ids = apply_filters('link_sawing_excluded_term_ids', array());
	...
	/**
	 * Get all post types supported by Link Sawing
	 *
	 * @param string $format
	 * @param string $cpt
	 * @param bool $include_user_excluded
	...
	/**
	 * Get all taxonomies supported by Link Sawing
	 *
	 * @param string $format
	 * @param string $tax
	 * @param bool $include_user_excluded
	...
	/**
	 * Get all post statuses supported by Link Sawing
	 */
	static function get_post_statuses()
	{
		$post_statuses = get_post_statuses();

		return apply_filters('link_sawing_post_statuses', $post_statuses);
	}
	...
	static function get_endpoints()
	{
		...
		return apply_filters("link_sawing_endpoints", str_replace("/", "\/", $endpoints));
	}
	...
	static function get_all_structure_tags($code = true, $separator = '', $hide_slug_tags = true)
	{
		...
		foreach ($tags as &$tag) {
			$tag = ($code) ? "<code class='ui label black' style='font-size:12px;font-family:tahoma;margin:5px 2px;'>{$tag}</code>" : "{$tag}";
		}
	...
	{
		return apply_filters('link_sawing_filter_permalink_base', trim(get_option('home'), "/"), $element);
	}
	...
	static function is_front_page($page_id)
	{
		...
		return apply_filters('link_sawing_is_front_page', $bool, $page_id, $front_page_id);
	}
	...
	static function is_advanced_mode_on()
	{
		...
		$bool = (!empty($link_sawing_options["general"]["advanced_mode"]) && $link_sawing_options["general"]["advanced_mode"] == 1) ? true : false;

		return apply_filters('link_sawing_advanced_mode_on', $bool);
	}
	...
	public static function sanitize_title($str, $keep_percent_sign = false, $force_lowercase = null, $sanitize_slugs = null)
	{
		...
		$force_lowercase = (!is_null($force_lowercase)) ? $force_lowercase : apply_filters('link_sawing_force_lowercase_uris', true);

		if (is_null($sanitize_slugs)) {
			$sanitize_slugs = (!empty($link_sawing_options['general']['disable_slug_sanitization'])) ? false : true;
		}

		// Allow to filter the slug before it is sanitized
		$str = apply_filters('link_sawing_pre_sanitize_title', $str, $keep_percent_sign, $force_lowercase, $sanitize_slugs);

		// Remove accents & entities
		$clean = (empty($link_sawing_options['general']['keep_accents'])) ? remove_accents($str) : $str;
		$clean = str_replace(array('&lt', '&gt', '&amp'), '', $clean);

		$percent_sign   = ($keep_percent_sign) ? "\%" : "";
		$sanitize_regex = apply_filters("link_sawing_sanitize_regex", "/[^\p{Xan}a-zA-Z0-9{$percent_sign}\/_\.|+, -]/ui", $percent_sign);
	...
	public static function replace_empty_placeholder_tags($default_uri, $native_slug = "", $element = "", $slug = "", $native_uri = false)
	{
		...
		$empty_tag_replacement = apply_filters('link_sawing_empty_tag_replacement', '', $element);
	...
	{
		global $link_sawing_options, $link_sawing_uris;

		$force_custom_slugs = (!empty($link_sawing_options['general']['force_custom_slugs'])) ? $link_sawing_options['general']['force_custom_slugs'] : false;
		$force_custom_slugs = apply_filters('link_sawing_force_custom_slugs', $force_custom_slugs, $slug, $object);
		...
		if ($force_custom_slugs) {
			...
			if ($force_custom_slugs == 1) {
				...
				$new_slug  = (!empty($link_sawing_uris[$object_id])) ? basename($link_sawing_uris[$object_id]) : '';
			}
	...
	public static function get_all_duplicates($include_custom_uris = true)
	{
		...
		$all_uris                    = ($include_custom_uris && is_array($link_sawing_uris)) ? $link_sawing_uris : array();
		$link_sawing_redirects = (is_array($link_sawing_redirects)) ? $link_sawing_redirects : array();

		// Convert redirects list, so it can be merged with $link_sawing_uris
	...
	public static function is_uri_duplicated($uri, $element_id, $duplicated_ids = array())
	{
		...
		if (empty($uri) || empty($element_id) || empty($link_sawing_uris)) {
			return false;
		}
		...
		} else if (in_array($uri, $link_sawing_uris)) {
			$all_duplicates = array_keys($link_sawing_uris, $uri);
		}

		if (!empty($all_duplicates)) {
			// Get the language code of current element
			$this_uri_lang = apply_filters('link_sawing_get_language_code', '', $element_id);
			...
			foreach ($all_duplicates as $key => $duplicated_id) {
				...
				if ($this_uri_lang) {
					$duplicated_uri_lang = apply_filters('link_sawing_get_language_code', '', $duplicated_id);
				}
	...
	public static function search_uri($search_query, $content_type = null)
	{
		...
		foreach ($link_sawing_uris as $id => $uri) {
	...
	{
		global $link_sawing_uris, $link_sawing_redirects, $link_sawing_external_redirects;

		if (function_exists('get_blog_option')) {
			$link_sawing_uris               = get_blog_option($new_blog_id, 'link-sawing-uris', array());
			$link_sawing_redirects          = get_blog_option($new_blog_id, 'link-sawing-redirects', array());
			$link_sawing_external_redirects = get_blog_option($new_blog_id, 'link-sawing-external-redirects', array());
		}
	}
}
