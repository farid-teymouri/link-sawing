 ...
 */
class link_sawing_Third_Parties
{
	...
	function init_hooks()
	{
		...
		if (defined('AMP_QUERY_VAR')) {
			add_filter('link_sawing_detect_uri', array($this, 'detect_amp'), 10, 2);
			add_filter('request', array($this, 'enable_amp'), 10, 1);
		}

		// 2. AMP for WP
		if (defined('AMPFORWP_AMP_QUERY_VAR')) {
			add_filter('link_sawing_filter_query', array($this, 'detect_amp_for_wp'), 5);
		}

		// 4. WooCommerce
		if (class_exists('WooCommerce')) {
			add_filter('link_sawing_filter_query', array($this, 'woocommerce_detect'), 9, 5);
			add_filter('template_redirect', array($this, 'woocommerce_checkout_fix'), 9);

			if (class_exists('link_sawing_Pro_Functions')) {
				if (empty($link_sawing_options['general']['partial_disable']['post_types']) || !in_array('shop_coupon', $link_sawing_options['general']['partial_disable']['post_types'])) {
					if (is_admin()) {
						add_filter('woocommerce_coupon_data_tabs', 'link_sawing_Pro_Functions::woocommerce_coupon_tabs');
						add_action('woocommerce_coupon_data_panels', 'link_sawing_Pro_Functions::woocommerce_coupon_panel');
						add_action('woocommerce_coupon_options_save', 'link_sawing_Pro_Functions::woocommerce_save_coupon_uri', 9, 2);
					}

					add_filter('request', 'link_sawing_Pro_Functions::woocommerce_detect_coupon_code', 1, 1);
					add_filter('link_sawing_disabled_post_types', 'link_sawing_Pro_Functions::woocommerce_coupon_uris', 9, 1);
				}
			}
			...
			add_filter('link_sawing_filter_default_post_uri', array($this, 'woocommerce_product_attributes'), 5, 5);

			if (wp_doing_ajax() && class_exists('SitePress')) {
				add_filter('link_sawing_filter_final_post_permalink', array($this, 'woocommerce_translate_ajax_fragments_urls'), 9999, 3);
			}
		}
		...
		if (defined('WPSEO_VERSION') && version_compare(WPSEO_VERSION, '14.0', '>=')) {
			add_action('link_sawing_updated_post_uri', array($this, 'yoast_update_indexable_permalink'), 10, 3);
			add_action('link_sawing_updated_term_uri', array($this, 'yoast_update_indexable_permalink'), 10, 3);
		...
		if (function_exists('tinv_get_option')) {
			add_filter('link_sawing_detect_uri', array($this, 'ti_woocommerce_wishlist_uris'), 15, 3);
		}
		...
		// 10. WP All Import
		if (class_exists('PMXI_Plugin') && (!empty($link_sawing_options['general']['pmxi_support']))) {
		...
		// 11. WP All Export
		if (class_exists('PMXE_Plugin') && (!empty($link_sawing_options['general']['pmxi_support']))) {
		...
		if (class_exists('\MyListing\Post_Types')) {
			add_filter('link_sawing_filter_default_post_uri', array($this, 'ml_listing_custom_fields'), 5, 5);
			add_action('mylisting/submission/save-listing-data', array($this, 'ml_set_listing_uri'), 100);
			add_filter('link_sawing_filter_query', array($this, 'ml_detect_archives'), 1, 2);
		}

		// 14. bbPress
		if (class_exists('bbPress') && function_exists('bbp_get_edit_slug')) {
			add_filter('link_sawing_endpoints', array($this, 'bbpress_endpoints'), 9);
		...
		if (class_exists('WeDevs_Dokan')) {
			...
			add_filter('link_sawing_endpoints', array($this, 'dokan_endpoints'));
		}

		// 16. GeoDirectory
		if (class_exists('GeoDirectory')) {
			add_filter('link_sawing_filter_default_post_uri', array($this, 'geodir_custom_fields'), 5, 5);
		}

		// 17. BasePress
		if (class_exists('Basepress')) {
			add_filter('link_sawing_filter_query', array($this, 'kb_adjust_query'), 5, 5);
		}

		// 18. Ultimate Member
		if (class_exists('UM') && !(empty($link_sawing_options['general']['um_support']))) {
			add_filter('link_sawing_detect_uri', array($this, 'um_detect_extra_pages'), 20);
		}

		// 19. WooCommerce Subscriptions
		if (class_exists('WC_Subscriptions')) {
			add_filter('link_sawing_filter_final_post_permalink', array($this, 'wcs_fix_subscription_links'), 10, 3);
		}

		// 20. LearnPress
		if (class_exists('LearnPress')) {
			add_filter('link_sawing_excluded_post_ids', array($this, 'learnpress_exclude_pages'));
		}
	}
	...
	public function init_early_hooks()
	{
		...
		// Woocommerce
		if (class_exists('WooCommerce')) {
			add_filter('woocommerce_get_endpoint_url', array('link_sawing_Core_Functions', 'control_trailing_slashes'), 9);
	...
	// {
	// 	global $link_sawing_uris, $link_sawing_before_sections_html;
	...
	// 			$link_sawing_uris[$item['id']] = link_sawing_Helper_Functions::sanitize_title($item_uri);
	// 		}

	// 		$link_sawing_before_sections_html .= link_sawing_Admin_Functions::get_alert_message(__('"Custom Permalinks" URIs were imported!', 'link-sawing'), 'updated');
	// 		update_option('link-sawing-uris', $link_sawing_uris);
	// 	} else {
	// 		$link_sawing_before_sections_html .= link_sawing_Admin_Functions::get_alert_message(__('No "Custom Permalinks" URIs were imported!', 'link-sawing'), 'error');
	// 	}
	// }
	...
	{
		global $wp_query, $pm_query, $link_sawing_options;

		// Redirect from Shop archive to selected page
		if (is_shop() && empty($pm_query['id'])) {
			$redirect_mode = (!empty($link_sawing_options['general']['redirect'])) ? $link_sawing_options['general']['redirect'] : false;
			$redirect_shop = apply_filters('link_sawing_redirect_shop_archive', false);
	...
	function woocommerce_generate_permalinks_after_duplicate($new_product, $old_product)
	{
		if (!empty($new_product)) {
			...
			if ($new_product->get_type() !== 'variation') {
				$custom_uri = link_sawing_URI_Functions_Post::get_default_post_uri($product_id, false, true);

				link_sawing_URI_Functions::save_single_uri($product_id, $custom_uri, false, true);
			}
		}
	}
	...
	function woocommerce_product_attributes($default_uri, $slug, $post, $post_name, $native_uri)
	{
		...
		if (!empty($custom_fields[1])) {
			...
			foreach ($custom_fields[1] as $i => $custom_field) {
				...
				$default_uri = str_replace($custom_fields[0][$i], link_sawing_Helper_Functions::sanitize_title($attribute_value), $default_uri);
			}
		}
	...
	function woocommerce_csv_custom_uri_column($columns)
	{
		...
		$label = __('آدرس سفارشی', 'link-sawing');
		$key   = 'custom_uri';
	...
	function woocommerce_export_custom_uri_value($value, $product, $column_id)
	{
		if (empty($value) && !empty($product)) {
			...
			$value = link_sawing_URI_Functions_Post::get_post_uri($product_id);
		}
	...
	function woocommerce_csv_import_custom_uri($product, $data)
	{
		...
		if (!empty($product)) {
			...
			if (empty($link_sawing_uris[$product_id]) && empty($data['custom_uri'])) {
				$custom_uri = link_sawing_URI_Functions_Post::get_default_post_uri($product_id, false, true);
			} else if (!empty($data['custom_uri'])) {
				$custom_uri = link_sawing_Helper_Functions::sanitize_title($data['custom_uri']);
			} else {
				return;
			}
	...
	function woocommerce_cot_compatibility()
	{
		if (class_exists('\Automattic\WooCommerce\Utilities\FeaturesUtil') && method_exists('\Automattic\WooCommerce\Utilities\FeaturesUtil', 'declare_compatibility')) {
			\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility('custom_order_tables', 'link-sawing');
			\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility('custom_order_tables', 'link-sawing-pro');
		}
	}
	...
	{
		global $wp, $link_sawing_ignore_permalink_filters;

		if (isset($wp->query_vars['action']) || !empty($_GET['redirect_to'])) {
			$link_sawing_ignore_permalink_filters = true;

			// Allow the canonical redirect (if blocked earlier by Link Sawing)
	...
	function yoast_update_indexable_permalink($element_id, $new_uri, $old_uri)
	{
		...
		if (!empty($new_uri) && !empty($old_uri) && $new_uri !== $old_uri) {
			if (current_filter() == 'link_sawing_updated_term_uri') {
	...
	function yoast_fix_canonical($url)
	{
		...
		if (!empty($pm_query['id'])) {
			...
			$url = link_sawing_Core_Functions::control_trailing_slashes($url);
		}
	...
	function filter_breadcrumbs($links)
	{
		...
		global $link_sawing_uris, $link_sawing_options, $post, $wpdb, $wp, $wp_current_filter;

		// Check if the filter should be activated
		if (empty($link_sawing_options['general']['yoast_breadcrumbs']) || empty($link_sawing_uris)) {
			return $links;
		}
		...
		if (!empty($element_id) && !empty($link_sawing_uris[$element_id])) {
			$custom_uri = preg_replace("/([^\/]+)$/", '', $link_sawing_uris[$element_id]);
		} else {
			$custom_uri = trim(preg_replace("/([^\/]+)$/", '', $wp->request), "/");
		}

		$all_uris                     = array_flip($link_sawing_uris);
		$custom_uri_parts             = explode('/', trim($custom_uri));
		$breadcrumbs                  = array();
		$snowball                     = '';
		$available_taxonomies         = link_sawing_Helper_Functions::get_taxonomies_array(null, null, true);
		$available_post_types         = link_sawing_Helper_Functions::get_post_types_array(null, null, true);
		$available_post_types_archive = link_sawing_Helper_Functions::get_post_types_array('archive_slug', null, true);
	...
	function ti_woocommerce_wishlist_uris($uri_parts, $request_url, $endpoints)
	{
		...
		if (is_numeric($wishlist_pid) && !empty($link_sawing_uris[$wishlist_pid])) {
			$wishlist_uri = preg_quote($link_sawing_uris[$wishlist_pid], '/');
	...
	function revisionize_keep_post_uri($old_id, $new_id)
	{
		...
		if (!empty($link_sawing_uris[$old_id])) {
			$link_sawing_uris[$new_id] = $link_sawing_uris[$old_id];
	...
	function revisionize_clone_uri($old_id, $new_id)
	{
		...
		if (!empty($link_sawing_uris[$new_id])) {
			$link_sawing_uris[$old_id] = $link_sawing_uris[$new_id];
			unset($link_sawing_uris[$new_id]);
	...
	function wpaiextra_uri_display($content_type, $current_values)
	{
		...
		if ($content_type !== 'taxonomies' && link_sawing_Helper_Functions::is_post_type_disabled($content_type)) {
			return;
		}
		...
		$html .= sprintf('<div class="wpallimport-collapsed-header"><h3>%s</h3></div>', __('Link Sawing', 'link-sawing'));
		$html .= '<div class="wpallimport-collapsed-content">';

		$html .= '<div class="template_input">';
		$html .= link_sawing_Admin_Functions::generate_option_field('custom_uri', array('extra_atts' => 'style="width:100%; line-height: 25px;"', 'placeholder' => __('آدرس سفارشی', 'link-sawing'), 'value' => $custom_uri));
		$html .= wpautop(sprintf(__('در صورت خالی بودن، پیوند پیش فرض شما بر اساس <a href="%s" target="_blank">تنظیمات ساختار پیوندها</a> شکل می گیرد.', 'link-sawing'), link_sawing_Admin_Functions::get_admin_url('&section=permastructs')));
		$html .= '</div>';
	...
	/**
	 * 10BB. Add Link Sawing plugin to the WP All Import API
	 *
	 * @param array $addons
	 *
	 * @return array
	...
	{
		if (empty($addons[link_sawing_PLUGIN_SLUG])) {
			$addons[link_sawing_PLUGIN_SLUG] = 1;
		}
	...
	/**
	 * 10BC. Register function that parses Link Sawing plugin data in WP All Import API data feed
	 *
	...
	{
		$functions[link_sawing_PLUGIN_SLUG] = array($this, 'wpai_api_parse_function');

		return $functions;
	}

	/**
	 * 10BD. Register function that saves Link Sawing plugin data extracted from WP All Import API data feed
	 *
	 * @param array $functions
	 *
	 * @return array
	...
	{
		$functions[link_sawing_PLUGIN_SLUG] = array($this, 'wpai_api_import_function');

		return $functions;
	}

	/**
	 * 10C. Parse Link Sawing plugin data in WP All Import API data feed
	 *
	 * @param array $data
	 *
	 * @return array
	...
	/**
	 * 10D. Save the Link Sawing plugin data extracted from WP All Import API data feed
	 *
	...
	function wpai_api_import_function($importData, $parsedData)
	{
		...
		if ($importData['post_type'] == 'taxonomies') {
			$is_term = true;
		} else if (link_sawing_Helper_Functions::is_post_type_disabled($importData['post_type'])) {
			return;
		}
		...
		if (isset($index) && !empty($pid) && !empty($parsedData['custom_uri'][$index])) {
			$new_uri = link_sawing_Helper_Functions::sanitize_title($parsedData['custom_uri'][$index]);

			if (!empty($new_uri)) {
				if (!empty($is_term)) {
					$default_uri = link_sawing_URI_Functions_Tax::get_default_term_uri($pid);
					$native_uri  = link_sawing_URI_Functions_Tax::get_default_term_uri($pid, true);
					$custom_uri  = link_sawing_URI_Functions_Tax::get_term_uri($pid, false, true);
					$old_uri     = (!empty($custom_uri)) ? $custom_uri : $native_uri;

					if ($new_uri !== $old_uri) {
						link_sawing_URI_Functions::save_single_uri($pid, $new_uri, true, true);
						do_action('link_sawing_updated_term_uri', $pid, $new_uri, $old_uri, $native_uri, $default_uri, $single_update = true, $uri_saved = true);
					}
				} else {
					$default_uri = link_sawing_URI_Functions_Post::get_default_post_uri($pid);
					$native_uri  = link_sawing_URI_Functions_Post::get_default_post_uri($pid, true);
					$custom_uri  = link_sawing_URI_Functions_Post::get_post_uri($pid, false, true);
					$old_uri     = (!empty($custom_uri)) ? $custom_uri : $native_uri;

					if ($new_uri !== $old_uri) {
						link_sawing_URI_Functions::save_single_uri($pid, $new_uri, false, true);
						do_action('link_sawing_updated_post_uri', $pid, $new_uri, $old_uri, $native_uri, $default_uri, $single_update = true, $uri_saved = true);
					}
				}
			}
		}
	...
	/**
	 * 10E. Copy the external redirect from the "external_redirect" custom field to the data model used by Link Sawing Pro
	 *
	...
	function wpai_save_redirects($pid)
	{
		...
		if ($external_url && class_exists('link_sawing_Pro_Functions')) {
			link_sawing_Pro_Functions::save_external_redirect($external_url, $pid);
		}
	}
	...
	function wpai_regenerate_uris_after_import($post_ids)
	{
		...
		foreach ($post_ids as $id) {
			...
			if (empty($post_object->post_type) || link_sawing_Helper_Functions::is_post_excluded($post_object, true)) {
				continue;
			}

			$default_uri = link_sawing_URI_Functions_Post::get_default_post_uri($id);
			link_sawing_URI_Functions::save_single_uri($id, $default_uri);
		}

		link_sawing_URI_Functions::save_all_uris();
	}
	...
	function wpae_custom_uri_section($sections)
	{
		if (is_array($sections)) {
			$sections['link_sawing'] = array(
				'title'   => __('Link Sawing', 'link-sawing'),
				'content' => 'link_sawing_fields'
			);
		}
	...
	/**
	 * 11B. Add a new field to the "Link Sawing" section of the WP All Export interface
	 *
	...
	{
		if (is_array($fields)) {
			$fields['link_sawing_fields'] = array(
				array(
					'label' => 'custom_uri',
					'name'  => 'Custom URI',
					'type'  => 'custom_uri'
	...
	function wpae_export_custom_uri($articles, $options)
	{
		...
		foreach ($articles as &$article) {
			...
			if (!empty($is_term)) {
				$article['Custom URI'] = link_sawing_URI_Functions_Tax::get_term_uri($item_id);
			} else {
				$article['Custom URI'] = link_sawing_URI_Functions_Post::get_post_uri($item_id);
			}
		}
	...
	function duplicate_custom_uri($new_post_id, $old_post)
	{
		...
		if (!empty($old_post->ID) && $duplicate_custom_uri_bool) {
			...
			if (!empty($link_sawing_uris[$old_post_id])) {
				$old_post_uri = $link_sawing_uris[$old_post_id];
				$new_post_uri = preg_replace('/(.+?)(\.[^\.]+$|$)/', '$1-2$2', $old_post_uri);

				$link_sawing_uris[$new_post_id] = $new_post_uri;
				update_option('link-sawing-uris', $link_sawing_uris);
			}
		}
	}
	...
	public function ml_listing_custom_fields($default_uri, $native_slug, $element, $slug, $native_uri)
	{
		...
		if (strpos($default_uri, '%listing-type%') !== false || strpos($default_uri, '%listing_type%') !== false) {
			...
			if (!empty($listing_type)) {
				$default_uri = str_replace(array('%listing-type%', '%listing_type%'), link_sawing_Helper_Functions::sanitize_title($listing_type, true), $default_uri);
			}
		}
		...
		if (strpos($default_uri, '%listing-type-slug%') !== false || strpos($default_uri, '%listing_type_slug%') !== false || strpos($default_uri, '%case27_listing_type%') !== false) {
			...
			if (!empty($listing_type)) {
				$listing_type = link_sawing_Helper_Functions::sanitize_title($listing_type, true);
		...
		if (strpos($default_uri, '%listing-location%') !== false || strpos($default_uri, '%listing_location%') !== false) {
			...
			if (!empty($listing_location)) {
				$listing_location = link_sawing_Helper_Functions::sanitize_title($listing_location, true);
		...
		if (strpos($default_uri, '%listing-region%') !== false || strpos($default_uri, '%listing_region%') !== false) {
			...
			$listing_region_term  = (!is_wp_error($listing_region_terms) && !empty($listing_region_terms) && is_object($listing_region_terms[0])) ? link_sawing_Helper_Functions::get_lowest_element($listing_region_terms[0], $listing_region_terms) : "";

			if (!empty($listing_region_term)) {
				$listing_region = link_sawing_Helper_Functions::get_term_full_slug($listing_region_term, $listing_region_terms, 2);
				$listing_region = link_sawing_Helper_Functions::sanitize_title($listing_region, true);
		...
		if (strpos($default_uri, '%listing-category%') !== false || strpos($default_uri, '%listing_category%') !== false) {
			...
			$listing_category_term  = (!is_wp_error($listing_category_terms) && !empty($listing_category_terms) && is_object($listing_category_terms[0])) ? link_sawing_Helper_Functions::get_lowest_element($listing_category_terms[0], $listing_category_terms) : "";

			if (!empty($listing_category_term)) {
				$listing_category = link_sawing_Helper_Functions::get_term_full_slug($listing_category_term, $listing_category_terms, 2);
				$listing_category = link_sawing_Helper_Functions::sanitize_title($listing_category, true);
	...
	function ml_set_listing_uri($post_id)
	{
		...
		if (!empty($link_sawing_uris)) {
			$default_uri = link_sawing_URI_Functions_Post::get_default_post_uri($post_id);

			if ($default_uri) {
				$link_sawing_uris[$post_id] = $default_uri;
				update_option('link-sawing-uris', $link_sawing_uris);
			}
		}
	}
	...
	/**
	 * 15A. Add the endpoint "edit" used by Dokan to the endpoints array supported by Link Sawing
	 *
	 * @param string $endpoints
	 *
	 * @return string
	...
	public function geodir_custom_fields($default_uri, $native_slug, $element, $slug, $native_uri)
	{
		...
		if (strpos($default_uri, '%category%') !== false) {
			...
			$place_category_term  = (!is_wp_error($place_category_terms) && !empty($place_category_terms) && is_object($place_category_terms[0])) ? link_sawing_Helper_Functions::get_lowest_element($place_category_terms[0], $place_category_terms) : "";

			if (!empty($place_category_term) && is_a($place_category_term, 'WP_Term')) {
				$place_category = link_sawing_Helper_Functions::get_term_full_slug($place_category_term, '', 2);
				$place_category = link_sawing_Helper_Functions::sanitize_title($place_category, true);
		...
		if (strpos($default_uri, '%country%') !== false && !empty($place_data->country)) {
			$place_country = link_sawing_Helper_Functions::sanitize_title($place_data->country, true);
			$default_uri   = str_replace('%country%', $place_country, $default_uri);
		}

		// C. Region
		if (strpos($default_uri, '%region%') !== false && !empty($place_data->region)) {
			$place_region = link_sawing_Helper_Functions::sanitize_title($place_data->region, true);
			$default_uri  = str_replace('%region%', $place_region, $default_uri);
		}

		// D. City
		if (strpos($default_uri, '%city%') !== false && !empty($place_data->city)) {
			$place_city  = link_sawing_Helper_Functions::sanitize_title($place_data->city, true);
	...
	public function um_detect_extra_pages($uri_parts)
	{
		...
		foreach ($um_pages as $um_page => $query_var) {
			...
			if (!empty($um_page_id) && !empty($link_sawing_uris[$um_page_id])) {
				$user_page_uri = preg_quote($link_sawing_uris[$um_page_id], '/');
				preg_match("/^({$user_page_uri})\/([^\/]+)?$/", $request_url, $parts);

				if (!empty($parts[2])) {
					$uri_parts['uri']            = $parts[1];
					$uri_parts['endpoint']       = $query_var;
					$uri_parts['endpoint_value'] = link_sawing_Helper_Functions::sanitize_title($parts[2], null, null, false);
				}
			}
		}
	...
	/**
	 * 20. Excluding the LearnPress pages from Link Sawing
	 *
