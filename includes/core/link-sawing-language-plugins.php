 ...
 */
class link_sawing_Language_Plugins
{
	...
	function init_hooks()
	{
		global $sitepress_settings, $polylang, $translate_press_settings, $link_sawing_options;
		...
		if ($sitepress_settings || !empty($polylang->links_model->options) || class_exists('TRP_Translate_Press')) {
			...
			add_filter('link_sawing_detected_post_id', array($this, 'fix_language_mismatch'), 9, 3);
			add_filter('link_sawing_detected_term_id', array($this, 'fix_language_mismatch'), 9, 3);

			// Fix posts page
			add_filter('link_sawing_filter_query', array($this, 'fix_posts_page'), 5, 5);

			// URI Editor
			add_filter('link_sawing_uri_editor_extra_info', array($this, 'uri_editor_get_lang_col'), 9, 3);

			// Adjust front page ID
			add_filter('link_sawing_is_front_page', array($this, 'wpml_is_front_page'), 9, 3);

			// Provide the language code for specific post/term
			add_filter('link_sawing_get_language_code', array($this, 'filter_get_language_code'), 9, 2);
			...
			if ($mode === 'prepend') {
				add_filter('link_sawing_detect_uri', array($this, 'detect_uri_language'), 9, 3);
				add_filter('link_sawing_filter_permalink_base', array($this, 'prepend_lang_prefix'), 9, 2);
			} else if ($mode === 'append') {
				add_filter('link_sawing_filter_final_post_permalink', array($this, 'append_lang_prefix'), 5, 2);
				add_filter('link_sawing_filter_final_term_permalink', array($this, 'append_lang_prefix'), 5, 2);
				add_filter('link_sawing_detect_uri', array($this, 'wpml_ignore_lang_query_parameter'), 9);
			}

			// Translate permastructures
			add_filter('link_sawing_filter_permastructure', array($this, 'translate_permastructure'), 9, 2);

			// Translate custom permalinks
			if ($this->is_wpml_compatible()) {
				add_filter('link_sawing_filter_final_post_permalink', array($this, 'translate_permalinks'), 9, 2);
			}

			// Translate post type slug
			if (class_exists('WPML_Slug_Translation')) {
				add_filter('link_sawing_filter_post_type_slug', array($this, 'wpml_translate_post_type_slug'), 9, 3);
			}
			...
			if (!empty($sitepress_settings['translation-management']) && !empty($sitepress_settings['translation-management']['doc_translation_method']) && $sitepress_settings['translation-management']['doc_translation_method'] == 'ATE') {
				if (apply_filters('link_sawing_ate_uri_editor', false) === true) {
			...
			if (!empty($polylang) && !empty($polylang->links_model) && !empty($polylang->links_model->options['hide_default'])) {
				add_filter('link_sawing_filter_query', array($this, 'pl_allow_canonical_redirect'), 3, 5);
			}
		}
	}

	/**
	 * Let users decide if they want Link Sawing to force language code in the custom permalinks
	 */
	public static function is_wpml_compatible()
	{
		...
		return (!empty($link_sawing_options['general']['wpml_support'])) ? $link_sawing_options['general']['wpml_support'] : false;
	}
	...
	{
		global $link_sawing_options, $link_sawing_uris, $pm_query, $polylang, $icl_adjust_id_url_filter_off;

		$mode = (!empty($link_sawing_options['general']['fix_language_mismatch'])) ? $link_sawing_options['general']['fix_language_mismatch'] : 0;
		...
		if ($detected_language_code !== $element_language_code) {
			...
			if ($mode == 1 || $mode == 2) {
				...
				if ($mode == 2 && $item_id !== $element_id) {
					$detected_element_uri   = link_sawing_URI_Functions::get_single_uri($element_id, false, false, $is_term);
					$translated_element_uri = link_sawing_URI_Functions::get_single_uri($item_id, false, false, $is_term);
	...
	{
		$language_code = self::get_language_code($element);
		$output        .= (!empty($language_code)) ? sprintf(" | <span><strong>%s:</strong> %s</span>", __("زبان"), $language_code) : "";
	...
	function wpml_ignore_lang_query_parameter($uri_parts)
	{
		...
		foreach ($link_sawing_uris as &$uri) {
			$uri = trim(strtok($uri, '?'), "/");
		}
	...
	/**
	 * Reapply WPML URL hooks and use them for custom permalinks filtered with Link Sawing
	 *
	 * @param string $permalink
	...
	{
		global $link_sawing_permastructs, $pagenow;
		...
		if (!empty($element->ID)) {
			$translated_permastructure = (!empty($link_sawing_permastructs["post_types"]["{$element->post_type}_{$language_code}"])) ? $link_sawing_permastructs["post_types"]["{$element->post_type}_{$language_code}"] : '';
		} else if (!empty($element->term_id)) {
			$translated_permastructure = (!empty($link_sawing_permastructs["taxonomies"]["{$element->taxonomy}_{$language_code}"])) ? $link_sawing_permastructs["taxonomies"]["{$element->taxonomy}_{$language_code}"] : '';
		}
	...
	function wpml_ate_edit_uri_field($elements, $post_id, $job_id)
	{
		...
		if (is_array($elements)) {
			...
			if (!empty($job->element_type)) {
				...
				$original_custom_uri = link_sawing_URI_Functions_Post::get_post_uri($post_id, true);

				if (!empty($translation_id)) {
					$translation_custom_uri   = link_sawing_URI_Functions_Post::get_post_uri($translation_id);
	...
	{
		add_filter('link_sawing_allow_new_post_uri', '__return_false');
	...
	function wpml_save_uri_after_wpml_translation_completed($post_id, $postdata, $job)
	{
		...
		if (!empty($postdata['Custom URI'])) {
			$new_uri = (!empty($postdata['Custom URI']['data']) && !in_array($postdata['Custom URI']['data'], array('-', 'auto'))) ? link_sawing_Helper_Functions::sanitize_title($postdata['Custom URI']['data']) : $default_uri;
		} // B. Generate the new custom permalink
		else if (empty($link_sawing_uris[$post_id])) {
			$new_uri = $default_uri;
		}

		// Save the custom permalink
		if (!empty($new_uri)) {
			link_sawing_URI_Functions::save_single_uri($post_id, $new_uri, false, true);
		}
	}
	...
	function wpml_cte_edit_uri_field($fields, $job)
	{
		...
		if (!empty($element_type)) {
			...
			$original_custom_uri = link_sawing_URI_Functions_Post::get_post_uri($original_id, true);

			if (!empty($translation_id)) {
				$translation_custom_uri   = link_sawing_URI_Functions_Post::get_post_uri($translation_id, true);
				$uri_translation_complete = (!empty($link_sawing_uris[$translation_id])) ? '1' : '0';
			} else {
	...
	function wpml_translation_save_uri($in = '', $data = '', $job = '')
	{
		...
		if (isset($data['pm-custom_uri']) && isset($data['pm-custom_uri']['data']) && !empty($translation_id)) {
			$custom_uri = (!empty($data['pm-custom_uri']['data'])) ? link_sawing_Helper_Functions::sanitize_title($data['pm-custom_uri']['data'], true) : link_sawing_URI_Functions_Post::get_default_post_uri($translation_id);

			link_sawing_URI_Functions::save_single_uri($translation_id, $custom_uri, false, true);
		}
	...
	function wpml_duplicate_uri($master_post_id, $lang, $post_array, $id)
	{
		...
		$link_sawing_uris[$id] = link_sawing_URI_Functions_Post::get_default_post_uri($id);
