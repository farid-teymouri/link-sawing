	...
	class Puc_v4p11_Plugin_Ui
	{
		...
		public function addViewDetailsLink($pluginMeta, $pluginFile, $pluginData = array())
		{
			if ($this->isMyPluginFile($pluginFile) && !isset($pluginData['slug'])) {
				$linkText = apply_filters($this->updateChecker->getUniqueName('view_details_link'), __('مشاهده جزئیات'));
				if (!empty($linkText)) {
					...
					$viewDetailsLink = sprintf(
						...
						esc_attr(sprintf(__('اطلاعات بیشتر در مورد %s'), $pluginData['Name'])),
		...
		public function addCheckForUpdatesLink($pluginMeta, $pluginFile)
		{
			if ($this->isMyPluginFile($pluginFile)) {
				...
				$linkText = apply_filters(
					...
					__('بررسی به روز رسانی ها', 'plugin-update-checker')
				);
				if (!empty($linkText)) {
		...
		public function displayManualCheckResult()
		{
			if (isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug)) {
				...
				if ($status == 'no_update') {
					...
					$message = sprintf(__('وضعیت به روز رسانی ناشناس "%s"', 'plugin-update-checker'), htmlentities($status));
					$noticeClass = 'error notice-error';
				}
				printf(
